...
class OVOSGuiPlayerService(AudioBackend):
    def __init__(self, config, bus=None, name='ovos_guiplayer'):
        super(OVOSGuiPlayerService, self).__init__(config, bus)
        self.config = config
        self.process = None
        self.bus = bus
        self.name = name
...
def load_service(base_config, bus):
    ...
    services = [(b, backends[b]) for b in backends
                if backends[b]['type'] == 'ovos_guiplayer' and
